...
public class NonPreemptiveSJF extends SchedulingAlgorithm{
    ...
    public static PriorityQueue<Process> queue = new PriorityQueue<Process>(new BurstComparator());
    public static ArrayList<TimeChart> timeChart = new ArrayList<TimeChart>();
    public static ArrayList<Integer> queueArrive = new ArrayList<Integer>();
    public static float avgTurnArdTime, avgWaitTime;

    public NonPreemptiveSJF(Scanner input){
        System.out.println("---------------------------");
    ...
    public static void simulation(int n){
        ...
        // add first process & all process that arrived earlier or same time into queue 
        for(i=1; (i<=n) && (proc2[i].arrivalTime<=nextVal); i++){
            queue.offer(new Process(true, i, proc2[i].burstTime, proc2[i].arrivalTime));
        }
...
// For process inside the queue (already arrive), sort process by burst time then by arrival time 
class BurstComparator implements Comparator<Process>{
    
    public int compare(Process p1, Process p2) {

        Integer compr = Integer.compare(p1.burstTime, p2.burstTime);
